...
def get_pie_chart(entered_site):                                    
    if entered_site == 'ALL':
        filtered_df = pd.DataFrame(spacex_df[['Launch Site','class']].groupby('Launch Site').sum()['class']).reset_index()
        ...
        site_data = spacex_df.groupby(['Launch Site', 'class']).count().reset_index()
        pie_data = site_data[site_data['Launch Site']==entered_site]
        fig = px.pie(pie_data, values='Flight Number', names='class', title='Total Success Launches for site '+ entered_site)
        return fig   
# TASK 4:
